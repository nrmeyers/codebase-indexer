...
def test_explorer_info_reports_unavailable_when_db_missing(tmp_path: Path) -> None:
    """When no per-repo DB exists, available must be False."""
    db_dir = str(tmp_path / "repos")
    with patch("app.routers.explorer.settings") as s1, patch("app.routers.health.settings") as s2:
        s1.LADYBUG_DB_DIR = db_dir
        s2.LADYBUG_DB_DIR = db_dir
    ...
    # Fallback path is derived from LADYBUG_DB_DIR + "graph.db".
    assert body["db_path"] == str(Path(db_dir) / "graph.db")


def test_explorer_info_reports_unavailable_when_db_empty(tmp_path: Path) -> None:
    """No indexed repos discovered — should still be unavailable (viewer would be empty)."""
    db_dir = tmp_path / "repos"
    db_dir.mkdir()

    with patch("app.routers.explorer.settings") as s1, patch("app.routers.health.settings") as s2, \
         patch("app.routers.explorer._get_indexed_repos", return_value=[]):
        s1.LADYBUG_DB_DIR = str(db_dir)
        s2.LADYBUG_DB_DIR = str(db_dir)
        client = TestClient(create_app())
        resp = client.get("/explorer/info")
...
def test_explorer_info_reports_available_when_indexed(tmp_path: Path) -> None:
    ...
    db_dir = tmp_path / "repos"
    db_dir.mkdir()
    db_path = str(db_dir / "myproject.db")
    Path(db_path).touch()
    ...
    with patch("app.routers.explorer.settings") as s1, patch("app.routers.health.settings") as s2, \
        ...
        # selected. Mock it to return the real per-repo path — without this,
        # MagicMock returns a MagicMock and ExplorerInfoResponse rejects it
        # with a string_type validation error.
...
def test_explorer_info_returns_launch_command_with_db_path(tmp_path: Path) -> None:
    ...
    assert str(db_dir) in body["launch_command"]
